...
class Tensor(np.ndarray):
    ...
    def __rpow__(self, other: Any) -> "Tensor":
        ...
        if type(other) in [np.ndarray, list]:
            ...
            out = Tensor(data=(other ** np.asarray(self.data)), _children=(self,), _op='**', use_grad=self._grad_enabled)
    ...
    def __getitem__(self, idx):
        s = np.asarray(self.data, dtype=self.dtype)
        if isinstance(idx, int):
            idx = [idx] + [slice(None, None, None) for _ in enumerate(self.shape[1:])]
